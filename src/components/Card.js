...
function Card(props) {
  ...
  return (
    <li className="cards__item" id={`${props.card.id}`}>
      ...
      <button className="cards__remove-button" type="button" aria-label="Удалить карточку" hidden={!isOwn}></button>
      <div className="cards__body">
        <h2 className="cards__label">{props.card.name}</h2>
        <div className="cards__like-container">
          <button className={`cards__like-button ${cardLikeButtonClassName}`} type="button" aria-label="Поставить лайк карточке"></button>
